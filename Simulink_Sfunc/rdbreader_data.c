...
#define S_FUNCTION_NAME rdbreader_data
...
static void mdlInitializeSizes(SimStruct *S)	//S function callback routine
{
    ...
    // port 4 holds up to 20 static object states of type RDB_ROADMARK_t (each with 76 bytes, i.e. 19 * 4 bytes)
    ssSetOutputPortWidth(    S, 3, 1 );
    ssSetOutputPortDataType( S, 3, SS_UINT32 );
    ssSetOutputPortWidth(    S, 4, 20 * 19 );
    ssSetOutputPortDataType( S, 4, SS_UINT32 );
        
    // RDB_LANE_INFO_t
    // port 5 holds the number of valid elements in port 6
    // port 6 holds up to 20 RDB_LANE_INFO_t (each with 56 bytes, i.e. 14 * 4 bytes)
    ...
    // RDB_ROAD_POS_t
    // port 7 holds the number of valid elements in port 12
    // port 8 holds up to 20 road position infos (each with 40 bytes, i.e. 10 * 4 bytes)
    ssSetOutputPortWidth(    S, 7, 1 );
    ssSetOutputPortDataType( S, 7, SS_UINT32 );
    ssSetOutputPortWidth(    S, 8, 20 * 10 );
    ssSetOutputPortDataType( S, 8, SS_UINT32 );
    
    // RDB_VEHICLE_SYSTEMS_t
    // port 11 holds the number of valid elements in port 14
    // port 12 holds up to 20 vehicle system infos (each with 44 bytes, i.e. 11 * 4 bytes)
    ssSetOutputPortWidth(    S, 11, 1 );
    ssSetOutputPortDataType( S, 11, SS_UINT32 );
    ssSetOutputPortWidth(    S, 12, 20 * 11 );
    ssSetOutputPortDataType( S, 12, SS_UINT32 );

    // RDB_CONTACT_POINT_t
    // port 13 holds the number of valid elements in port 14
    // port 14 holds up to 20 contact point(each with 56 bytes, i.e. 14 * 4 bytes)
    ssSetOutputPortWidth(    S, 13, 1 );
    ssSetOutputPortDataType( S, 13, SS_UINT32 );
    ssSetOutputPortWidth(    S, 14, 20 * 14 );
    ssSetOutputPortDataType( S, 14, SS_UINT32 );

    // RDB_ENVIRONMENT_t
    // port 15 holds the number of valid elements in port 8
    // port 16 holds up to 1 environment state info (each with 48 bytes, i.e. 12 * 4 bytes)
    ssSetOutputPortWidth(    S, 15, 1 );
    ssSetOutputPortDataType( S, 15, SS_UINT32 );
    ssSetOutputPortWidth(    S, 16, 1 * 12 );
    ssSetOutputPortDataType( S, 16, SS_UINT32 );
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    unsigned int *inFrameData    = (unsigned int)ssGetInputPortSignal( S, 1 );
    ...
    unsigned int *inPtr2 = (unsigned int*) ssGetInputPortSignal( S, 2 );
	outData[199] = inData[2];
	int i=0;
	int port_no=1;
  int data_l; 
	while(port_no)
     ...
     *pkgID 	= inData[13+noPkg];
     ...
     //outData[3] = *pkgID;
      ...
      unsigned int *tgtScalar = ssGetOutputPortSignal( S, port_no );
			unsigned int *tgtVec    = ssGetOutputPortSignal( S, port_no + 1 );
			unsigned int maxSize    = ssGetOutputPortWidth( S, port_no + 1 ) * sizeof( unsigned int );
      /*
      unsigned int *tgtScalar = ssGetOutputPortSignal( S, 9 );
			unsigned int *tgtVec    = ssGetOutputPortSignal( S, 9 + 1 );
			unsigned int maxSize    = ssGetOutputPortWidth( S, 9 + 1 ) * sizeof( unsigned int );
      */
			tgtScalar[0] = inData[12+noPkg];		
      data_l = (inData[12+noPkg])/4;
      for(unsigned int j=0;j<data_l;j++)
      {
        tgtVec[j]=inData[14+noPkg+j];
      }

			
      outData[i] = tgtScalar[0];//inData[12+noPkg];
      outData[i+1] = port_no;
      outData[i+2] = *pkgID;
      i+=3;
      noPkg +=data_l +4;

...
#define S_FUNCTION_NAME rdbreader_vtd
...
static unsigned int sNetworkStreamNoElements  = sizeof( MY_RDB_MSG_t )/4;   // e.g. 38 uints @ 4 bytes each
...
//static unsigned int RDB_OBJECT_STATE_t_No =sizeof(RDB_OBJECT_STATE_t);
...
static void mdlInitializeSizes(SimStruct *S)	//S function callback routine
{
    ...
    // RDB_OBJECT_STATE_t
    // port 1 holds the number of valid elements in port 1
    // port 2 holds up to 1 driver control infos (each with 80 bytes, i.e. 20 * 4 bytes)
    ssSetOutputPortWidth(    S, 1, 1 );
    ssSetOutputPortDataType( S, 1, SS_UINT32 );
    ssSetOutputPortWidth(    S, 2, 52 );
    ssSetOutputPortDataType( S, 2, SS_UINT32 );
    
    // RDB_ROADMARK_t
    // port 3 holds the number of valid elements in port 1
    // port 24holds up to 1 driver control infos (each with 80 bytes, i.e. 20 * 4 bytes)
    ssSetOutputPortWidth(    S, 3, 1 );
    ssSetOutputPortDataType( S, 3, SS_UINT32 );
    ssSetOutputPortWidth(    S, 4, 19 );
    ssSetOutputPortDataType( S, 4, SS_UINT32 );
    
    // RDB_LANE_INFO_t
    // port 5 holds the number of valid elements in port 1
    // port 6 holds up to 1 driver control infos (each with 80 bytes, i.e. 20 * 4 bytes)
    ssSetOutputPortWidth(    S, 5, 1 );
    ssSetOutputPortDataType( S, 5, SS_UINT32 );
    ssSetOutputPortWidth(    S, 6, 14 );
    ssSetOutputPortDataType( S, 6, SS_UINT32 );
    
    // RDB_ROAD_POS_t
    // port 7 holds the number of valid elements in port 1
    // port 8 holds up to 1 driver control infos (each with 80 bytes, i.e. 20 * 4 bytes)
    ssSetOutputPortWidth(    S, 7, 1 );
    ssSetOutputPortDataType( S, 7, SS_UINT32 );
    ssSetOutputPortWidth(    S, 8, 10 );
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    //RDB_OBJECT_STATE_t
    unsigned int *tgtScalar = ssGetOutputPortSignal( S, 1 );
    unsigned int *tgtVec    = ssGetOutputPortSignal( S, 2 );
    unsigned int maxSize    = ssGetOutputPortWidth( S,  2 ) * sizeof( unsigned int );
    *tgtScalar = sizeof(RDB_OBJECT_STATE_t);
    for (int i=0;i<*tgtScalar;i++)
    {
        tgtVec[i] = inData[i+14];
    }
    
    //RDB_ROADMARK_t
      unsigned int *tgtScalar1 = ssGetOutputPortSignal( S, 3 );
      unsigned int *tgtVec1    = ssGetOutputPortSignal( S, 4 );
      unsigned int maxSize1   = ssGetOutputPortWidth( S,  4 ) * sizeof( unsigned int );
    *tgtScalar1 = sizeof(RDB_ROADMARK_t);
    for (int i=0;i<*tgtScalar1;i++)
    {
        tgtVec1[i] = inData[i+70];
    }
    
    //RDB_LANE_INFO_t
      unsigned int *tgtScalar2 = ssGetOutputPortSignal( S, 5 );
      unsigned int *tgtVec2   = ssGetOutputPortSignal( S, 6 );
      unsigned int maxSize2    = ssGetOutputPortWidth( S,  6 ) * sizeof( unsigned int );
     *tgtScalar2 = sizeof(RDB_LANE_INFO_t);
    for (int i=0;i<*tgtScalar2;i++)
    {
        tgtVec2[i] = inData[i+93];
    }
    
    //RDB_ROAD_POS_t
      unsigned int *tgtScalar3 = ssGetOutputPortSignal( S, 7 );
      unsigned int *tgtVec3    = ssGetOutputPortSignal( S, 8 );
      unsigned int maxSize3    = ssGetOutputPortWidth( S,  8 ) * sizeof( unsigned int );
       *tgtScalar3 = sizeof(RDB_ROAD_POS_t);
    for (int i=0;i<*tgtScalar3;i++)
    {
        tgtVec3[i] = inData[i+111];
    }
